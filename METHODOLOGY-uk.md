...
financial statements, cash flow, balance sheet, peer snapshots, SPY як
...
| ACTUAL_PEERS група відсутня і використано Yahoo fallback | +2 |
...
- `riskFlags` - чому саме;
- peer group - ACTUAL_PEERS є дефолтним джерелом; якщо тікера там немає,
  Yahoo fallback peers є тільки стартовим наближенням.
